/* Task Manager · JS icon helper
   Mirrors the SVG set used by {% icon %} in templates so client-rendered
   UI (palette results, toasts) stays visually identical. */
...
(function () {
    ...
    var FALLBACK = '<circle cx="12" cy="12" r="9" />';

    TM.icon = function (name, size, cls) {
        var body = BODIES[name] || FALLBACK;
        size = size || 18;
        cls = "icon" + (cls ? " " + cls : "");
